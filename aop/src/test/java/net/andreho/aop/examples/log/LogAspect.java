package net.andreho.aop.examples.log;

import net.andreho.aop.Aspect;
import net.andreho.aop.Forwarding;
import net.andreho.aop.Modify;
import net.andreho.aop.injectable.Arg;
import net.andreho.aop.injectable.Attribute;
import net.andreho.aop.spec.Annotated;
import net.andreho.aop.spec.ClassSelector;
import net.andreho.aop.spec.Classes;
import net.andreho.aop.spec.Methods;
import net.andreho.aop.spec.Modifier;
import net.andreho.aop.spec.Modifiers;
import net.andreho.aop.spec.Named;
import net.andreho.aop.spec.Scope;
import net.andreho.asm.org.objectweb.asm.Type;
import net.andreho.haxxor.Haxxor;
import net.andreho.haxxor.cgen.HxInstruction;
import net.andreho.haxxor.cgen.HxInstructionFactory;
import net.andreho.haxxor.cgen.HxInstructions;
import net.andreho.haxxor.spec.api.HxAnnotation;
import net.andreho.haxxor.spec.api.HxCode;
import net.andreho.haxxor.spec.api.HxField;
import net.andreho.haxxor.spec.api.HxMethod;
import net.andreho.haxxor.spec.api.HxType;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

import java.util.Optional;

/**
 * <br/>Created by a.hofmann on 23.03.2017 at 19:34.
 */
@Aspect(
    scope = {Scope.TYPE},
    classes = @Classes(
        @ClassSelector(
            modifiers = @Modifier(value = Modifiers.ABSTRACT | Modifiers.INTERFACE, negate = true),
            annotated = @Annotated(TransparentLog.class)
        )
    )
)
public class LogAspect {
  public static final String LOGGER_ATTRIBUTE_NAME = "TransparentLog";

  @Modify
  public static boolean setupLog(final HxType hxType) {
    Optional<HxAnnotation> optional = hxType.getAnnotation(TransparentLog.class);
    if (!optional.isPresent()) {
      return false;
    }

    HxAnnotation annotation = optional.get();
    String logFieldName = annotation.attribute("value");

    if (hxType.hasField(logFieldName)) {
      return false;
    }

    final Haxxor haxxor = hxType.getHaxxor();
    final HxField field = haxxor.createField(Logger.class, logFieldName)
                                .setModifiers(HxField.Modifiers.PUBLIC, HxField.Modifiers.STATIC, HxType.Modifiers.FINAL);

    HxAnnotation hxAnnotation = haxxor.createAnnotation(Attribute.class, true);
    hxAnnotation.attribute("value", LOGGER_ATTRIBUTE_NAME);
    field.addAnnotation(hxAnnotation);
    hxType.addField(field);

    Optional<HxMethod> methodOptional = hxType.findMethod("<clinit>");
    HxMethod clinit;
    if(methodOptional.isPresent()) {
      clinit = methodOptional.get();
    } else {
      clinit = haxxor.createMethod("void", "<clinit>");
      clinit.getCode().build().RETURN();
      hxType.addMethod(clinit);
    }

    HxCode code = clinit.getCode();
    HxInstructionFactory factory = code.getInstructionFactory();
    Optional<HxInstruction> returnOptional =
        code.getLast().findLastWithType(HxInstructions.Exit.RETURN);

    HxInstruction returnInst = returnOptional.get();
    returnInst.getPrevious()
              .append(factory.LDC(hxType.getName()))
              .append(factory.INVOKESTATIC(
                  Type.getInternalName(LoggerFactory.class),
                  "getLogger",
                  "("+Type.getDescriptor(String.class)+")"+Type.getDescriptor(Logger.class), false))
              .append(factory.PUTSTATIC(hxType.toInternalName(), field.getName(), field.toDescriptor()));

    return true;
  }

  @Forwarding(@Methods(declaredBy = @Classes(@ClassSelector(Logging.class)), named = @Named("info")))
  public static void info(@Arg String message, @Attribute(LOGGER_ATTRIBUTE_NAME) Logger logger) {
    logger.info(message);
  }

  @Forwarding(@Methods(declaredBy = @Classes(@ClassSelector(Logging.class)), named = @Named("trace")))
  public static void trace(@Arg String message, @Attribute(LOGGER_ATTRIBUTE_NAME) Logger logger) {
    logger.trace(message);
  }

  @Forwarding(@Methods(declaredBy = @Classes(@ClassSelector(Logging.class)), named = @Named("debug")))
  public static void debug(@Arg String message, @Attribute(LOGGER_ATTRIBUTE_NAME) Logger logger) {
    logger.debug(message);
  }

  @Forwarding(@Methods(declaredBy = @Classes(@ClassSelector(Logging.class)), named = @Named("warn")))
  public static void warn(@Arg String message, @Attribute(LOGGER_ATTRIBUTE_NAME) Logger logger) {
    logger.warn(message);
  }

  @Forwarding(@Methods(declaredBy = @Classes(@ClassSelector(Logging.class)), named = @Named("error")))
  public static void error(@Arg String message, @Attribute(LOGGER_ATTRIBUTE_NAME) Logger logger) {
    logger.error(message);
  }

  @Forwarding(@Methods(declaredBy = @Classes(@ClassSelector(Logging.class)), named = @Named("fatal")))
  public static void fatal(@Arg String message, @Attribute(LOGGER_ATTRIBUTE_NAME) Logger logger) {
    logger.error(message);
  }
}
