 ...
 */
public class HxAbstractType extends HxAnnotatedImpl<HxType> implements HxType {
   ...
   public HxMethod getMethod(String name, String... signature) {
      ...
      for (HxMethod method : methods) {
         ...
         for (int i = 0, len = method.getArity(); i < len; i++) {
            ...
            if (!type.getName().equals(signature[i])) {
               continue loop;
            }
         }
   ...
   public boolean isAssignableFrom(HxType otherType) {
      ...
      if (isArray()) {
         ...
         return this.getDimension() == otherType.getDimension() &&
                this.getComponentType().isAssignableFrom(otherType.getComponentType());
      }
   ...
   public String getSimpleBinaryName() {
      ...
      try {
         return getName().substring(enclosingType.getName().length());
   ...
   @Override
   public StringBuilder toDescriptor(StringBuilder builder) {
      HxType type = this;
      ...
      return builder.append('L').append(type.getName().replace(',', '/')).append(';');
   }
